...
def spatial_kfold_clusters(gdf:gpd.GeoDataFrame, name:str, nfolds:int, algorithm='kmeans', random_state=None, **kwargs):
    ...
    if algorithm == 'kmeans':
        clustering_model = KMeans(n_clusters=nfolds, random_state=random_state, **kwargs)
    elif algorithm == 'bisectingkmeans':
        clustering_model = BisectingKMeans(n_clusters=nfolds, random_state=random_state, **kwargs)            
